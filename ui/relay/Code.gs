...
const WORKER_URL_RAW = "https://example.workers.dev";
// خودترمیم: اگر آدرس با https:// اضافه یا اسلشِ آخر وارد شده باشد، اینجا تمیز می‌شود
// (قبلاً همین باعثِ خطای «DNS error: https://https://…» می‌شد).
const WORKER_URL = "https://" + String(WORKER_URL_RAW)
  .replace(/^\s+|\s+$/g, "")
  .replace(/^https?:\/\//i, "")
  .replace(/^https?:\/\//i, "")
  .replace(/\/+$/, "");
